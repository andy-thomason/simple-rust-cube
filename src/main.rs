...
fn main() {
    for frame_number in 0.. {
        ...
        // let mut world_pos = [Vector([0.0, 0.0, 0.0, 0.0]); 8];
...
fn test_draw_line() {
    let mut frame = [[b' ';SCREEN_WIDTH]; SCREEN_HEIGHT];
    // draw_line(&mut frame, [10.0, 0.0], [15.0, 10.0]);
    draw_line(&mut frame, [62.5, 28.125],[75.0, 31.25]);
    draw_line(&mut frame, [75.0, 31.25], [25.0, 31.25]);
    for l in 0..SCREEN_HEIGHT {
        let row = std::str::from_utf8(&frame[l]).unwrap();
        println!("{}", row);
    }
